...
class QuizCreatorTests extends FlatSpec with Matchers {
  ...
  it should "find a quiz word and split the article according to it" in {
    ...
    assert(quiz === List(PlainText("article "), QuizWord(0, "of"), PlainText(" words")))
  }

  it should "find multiple quiz words" in {
    val quiz = createQuiz about List("at", "am", "and") from "I am good at maths and chemistry"
    assert(quiz === List(PlainText("I "), QuizWord(0, "am"), PlainText(" good "), QuizWord(1, "at"), PlainText(" maths "), QuizWord(2, "and"), PlainText(" chemistry")))
  }

  it should "be case-insensitive" in {
    val quiz = createQuiz about List("of") from "Yes. Of course!"
    assert(quiz === List(PlainText("Yes. "), QuizWord(0, "Of"), PlainText(" course!")))
  }
  ...
  it should "identify quiz word if it's the last word in the article" in {
    ...
    assert(quiz === List(PlainText("come "), QuizWord(0, "by")))
  }
  ...
  it should "identify quiz word if there's punctuation after it" in {
    ...
    assert(quiz === List(QuizWord(0, "Morning"), PlainText(", "), QuizWord(1, "noon"), PlainText(". "), QuizWord(2, "Evening"), PlainText("? "), QuizWord(3, "Night"), PlainText("!")))
  }
}
